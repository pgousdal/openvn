...
OpenVNCommand openvn_command_from_string(const char *name) {
    ...
    if (strcmp(name, "LOAD") == 0) {
        return OPENVN_CMD_LOAD;
    }
    if (strcmp(name, "RUN") == 0) {
        return OPENVN_CMD_RUN;
    }
    if (strcmp(name, "SCENE") == 0) {
        return OPENVN_CMD_SCENE;
    }
    if (strcmp(name, "SHOW") == 0) {
        return OPENVN_CMD_SHOW;
    }
    if (strcmp(name, "HIDE") == 0) {
        return OPENVN_CMD_HIDE;
    }
    if (strcmp(name, "MUSIC") == 0) {
        return OPENVN_CMD_MUSIC;
    }
    if (strcmp(name, "SOUND") == 0) {
        return OPENVN_CMD_SOUND;
    }
    if (strcmp(name, "QUIT") == 0) {
        return OPENVN_CMD_QUIT;
    }
...
int openvn_parse_request(const char *line, OpenVNRequest *request) {
    ...
    if (request->command == OPENVN_CMD_INVALID) {
        return 0;
    }
    ...
    switch (request->command) {
        ...
        case OPENVN_CMD_SHOW:
            return request->argument1[0] != '\0' && request->argument2[0] != '\0';
        case OPENVN_CMD_RUN:
        case OPENVN_CMD_QUIT:
            return 1;
        default:
